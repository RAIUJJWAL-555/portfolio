...
export default function InfiniteMenu({ items, scale = 3.0 }) {
  const canvasRef = useRef(null);
  const [activeItem, setActiveItem] = useState(null);
  const [isMoving, setIsMoving] = useState(false);
  const internalItems = React.useMemo(() => items || defaultItems, [items]);
  ...
  useEffect(() => {
    ...
    const handleActiveItem = index => {
      const itemIndex = index % internalItems.length;
      setActiveItem(internalItems[itemIndex]);
    };
